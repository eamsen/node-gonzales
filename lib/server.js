...
CONFIG.getConfigSources().forEach(function(c) {
  console.log("Using configuration: " + c.name);
});
...
var SpdyProxy = function(options) {
  ...
  function requestTimeEnd(label) {
    var end = now();
    var diff = (end - this._timers[label]);

    this.log('\'%s\' took %dms', label, diff.toFixed(2));
  }
  ...
  function handleRequest(request, response) {
    ...
    request.timeEnd = requestTimeEnd;

    console.log('%s\t%s',
        new Date().toISOString().time,
        request.url);

    if (plugins.handleRequest(request, response, CONFIG)) {
    ...
    var forwardRequest = http.request(httpOpts, function(forwardResponse) {
      ...
      request.time = requestTime;
      request.timeEnd = requestTimeEnd;
      plugins.handleResponse(request, forwardResponse, response, CONFIG);
    });
...
var proxy = new SpdyProxy({
  key: fs.readFileSync(CONFIG.server.keyPath),
  cert: fs.readFileSync(CONFIG.server.certPath),
  port: CONFIG.server.proxyPort
});

proxy.listen(CONFIG.server.proxyPort);

var pacServer = new PacServer({ port: CONFIG.server.proxyPort });
pacServer.listen(CONFIG.server.pacPort);
